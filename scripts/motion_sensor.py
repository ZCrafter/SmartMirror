...
POLL_S     = float(os.getenv('MOTION_POLL_SECONDS', '0.25'))
HEARTBEAT_S = float(os.getenv('MOTION_HEARTBEAT_SECONDS', '5'))
    ...
    with gpiod.request_lines(
        ...
        while True:
            ...
            if value == Value.ACTIVE:
                if last_state == Value.INACTIVE:
                    log('MOTION DETECTED (GPIO HIGH)')
                # Refresh the server while the PIR output remains HIGH. Without
                # this heartbeat, repeatable-trigger mode can hold the line
                # high while the server's grace timer incorrectly expires.
                if last_state == Value.INACTIVE or now - last_notify_at >= HEARTBEAT_S:
                    notify_motion()
                    last_notify_at = now
            elif last_state == Value.ACTIVE:
                log('MOTION ENDED (GPIO LOW)')
