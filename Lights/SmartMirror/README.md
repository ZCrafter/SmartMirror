...
- GPIO 4
- 566 LEDs (adjust the layout values in `.env` if the strip changes)
- DDP UDP port 4048
...
The project-level `setup.sh` installs this renderer as
`mirror-lights.service`. Do not also launch `main.py` manually, because two
weather senders would compete for WLED realtime control.

Logs:

```bash
journalctl -u mirror-lights.service -f
```
